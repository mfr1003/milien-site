...
STEP 3 — GET THE DOCUMENT NOTARIZED (what notarization means, where to go in ${module.name})
STEP 4 — CONFIRM THE LEGAL DESCRIPTION (how to look it up at ${propertyCounty} County ${module.filingOffice})
STEP 5 — FILE WITH THE ${propertyCounty.toUpperCase()} COUNTY ${module.filingOffice.toUpperCase()} (process, fees ~$15-30, get stamped copy)
${stateCode === 'OH' ? 'STEP 6 — SERVE COPY ON PROPERTY OWNER within 30 days of filing (required in Ohio)\n' : ''}STEP ${stateCode === 'OH' ? '7' : '6'} — WHAT IF THEY STILL DON\'T PAY (lien as leverage, enforcement requires attorney, 1 year deadline${stateCode === 'OH' ? ' or 6 years' : ''})
...
function buildAutoDocumentPrompt(stateCode, metadata) {
  const module = AUTO_MODULES[stateCode];
  if (!module) throw new Error(`Automotive lien not supported for state: ${stateCode}`);
  ...
  const todayDate = new Date();
  const daysElapsed = Math.floor((todayDate - lastDate) / (1000 * 60 * 60 * 24));
  const daysRemaining = 105 - daysElapsed;
  const filingDeadline = new Date(lastDate);
  filingDeadline.setDate(filingDeadline.getDate() + 105);
  const filingDeadlineStr = filingDeadline.toLocaleDateString('en-US', {
    year: 'numeric', month: 'long', day: 'numeric'
  });

  const prompt = `You are a Michigan Garage Keeper's Lien document preparation specialist with deep knowledge of the Michigan Garage Keeper's Lien Act (MCL 570.301 et seq.) and the Motor Vehicle Service and Repair Act (MCL 257.1301 et seq.).

APPLICABLE LAW: Michigan Garage Keeper's Lien Act, MCL 570.301 et seq.
FILING AGENCY: Michigan Department of State (MDOS) — form BDVR-35
...
KEY MICHIGAN GARAGE KEEPER'S LIEN REQUIREMENTS:
- Claimant must be a licensed repair facility under the Motor Vehicle Service and Repair Act (MCL 257.1301)
